...
import 'package:last_exam/ui/cards/widgets/drop_down.dart';
import 'package:last_exam/ui/cards/widgets/list_view.dart';
import 'package:last_exam/ui/cards/widgets/text_field.dart';
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const ColorsView(),
            CustomTextField(text: "Karta raqami", type: TextInputType.number,format: MyConstants.cardFormat),
            const SizedBox(height: 16),
            CustomTextField(
                text: "Amal qilish muddati", type: TextInputType.datetime,format: MyConstants.validityPeriod,),
            const SizedBox(height: 16),
            CustomTextField(text: "Karta nomi", type: TextInputType.text,format: ""),
            const SizedBox(height: 16),
            CustomTextField(
            ...
            InkWell(
                onTap: () async {
                 if(context.read<EditCardProvider>().validateAllFields()){
                   context.read<EditCardProvider>().changeFields(isLoading1: true);
                   ...
                   context.read<EditCardProvider>().changeFields(isLoading1: false);
                   context.read<EditCardProvider>().refreshFields();
                   MyToast("Your card added successfully!");
                 }else{
                   MyToast("Fill all gaps!");
                 }
                },
                child: CostumButton(text: "Qo'shish",)),
          ],
        ),
      ),
    );
